/*--------------------------------------------------------------
  Adapted from 04_split_train_test.sas in this repo.
  Logic unchanged: stratified 70/30 TRAIN/TEST split with
  PROC SURVEYSELECT, then bank-style audit counts.
  Only change: input is the inline real-data sample below
  (a deterministic slice of scored_sample.csv) instead of
  work.lc_model_final from the upstream pipeline.
--------------------------------------------------------------*/
