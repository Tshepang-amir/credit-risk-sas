/*--------------------------------------------------------------
  Adapted from 06_model_tree_hpsplit.sas in this repo.
  Logic unchanged: train a PROC HPSPLIT decision tree (entropy
  growth, cost-complexity pruning), export scoring code, then
  %include it to score the data and standardise PD = P(default=1).
  Only changes: input is the inline real-data sample below
  (a deterministic slice of scored_sample.csv), and the score-
  code file is written/included by relative name.
--------------------------------------------------------------*/
...
/* Consistent variable lists (mirrors 06_model_tree_hpsplit.sas) */
%let interval_vars =
  term_m int_rate_num installment dti_clean fico_mid loan_amnt
  revol_util_clean log_annual_inc open_acc total_acc revol_bal;

%let nominal_vars =
  home_ownership verification_status purpose
  grade sub_grade application_type addr_state;

/*--------------------------------------------------------------
  1) Train tree + export scoring code
--------------------------------------------------------------*/
proc hpsplit data=work.lc_sample seed=&seed assignmissing=similar;
  ...
  code file="tree_score_code.sas";
run;

/*--------------------------------------------------------------
  2) Score with generated scoring code; standardise PD = P(default=1)
--------------------------------------------------------------*/
data work.score_tree_test;
  set work.lc_sample;
  %include "tree_score_code.sas";
  PD = P_default1;
  keep &target PD;
run;

title "Decision Tree (HPSPLIT) - scored PD (head)";
proc print data=work.score_tree_test(obs=10) noobs; run;
title;

/*--------------------------------------------------------------
  3) Confusion matrix at cutoff (bank-style operating point)
--------------------------------------------------------------*/
%let cutoff = 0.20;
...
title "Decision Tree (HPSPLIT) - Confusion Matrix (Cutoff=&cutoff)";
