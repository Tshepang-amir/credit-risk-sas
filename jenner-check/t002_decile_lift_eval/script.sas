/*--------------------------------------------------------------
  Adapted from the evaluation core of 05_logistic_baseline.sas
  and 07_model_evaluation.sas in this repo.
  Logic unchanged: confusion matrix at the chosen cutoff, then
  PROC RANK deciles + PROC SQL decile table with lift and
  cumulative bad capture.
  PD here is P_1 from the scored data (exactly what the pipeline
  writes as the probability of default), read from the inline
  real-data sample below instead of work.scored_test.
--------------------------------------------------------------*/
...
/* Standardise PD = P_1 (probability of default), as in 05 step 4 */
